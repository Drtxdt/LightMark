...
export function renderMarkdownForEditor(markdown) {
    return editorMd.render(markSpecialBlocksForEditor(markdown));
}
export function buildExportHtml(title, body) {
    return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
...
function markSpecialBlocksForEditor(markdown) {
    const placeholders = [];
    const stash = (html) => {
...
function enhanceMarkdownForRender(markdown) {
    let next = markdown.replace(/^---\s*\n([\s\S]*?)\n---\s*(?=\n|$)/, (_match, yaml) => {
...
function installLightMarkMarkdown(instance) {
...
function renderInlineEnhancements(html) {
    return html
        .replace(/==([^=\n]+)==/g, "<mark>$1</mark>")
...
function convertTaskItems(markdown) {
...
function convertFootnotes(markdown, stash, forEditor = false) {
    ...
    next = next.replace(/\[\^([^\]]+)\]/g, (_match, id) => {
        ...
        if (!referenceOrder.has(id))
            referenceOrder.set(id, referenceOrder.size + 1);
        const order = referenceOrder.get(id) || 1;
        const count = (referenceCounts.get(id) || 0) + 1;
        referenceCounts.set(id, count);
        if (forEditor) {
            return `<span data-type="footnote-ref" data-footnote-ref="${safeId}" data-footnote-index="${order}" data-ref-id="fnref-${safeId}-${count}"></span>`;
        }
        return `<sup data-footnote-ref="${safeId}" data-footnote-index="${order}"><a href="#fn-${safeId}" id="fnref-${safeId}-${count}" data-footnote-link="ref">[${order}]</a></sup>`;
    });
    ...
    const source = Array.from(definitions.entries())
        ...
        .join("\n\n");
    const section = `<section class="footnotes" data-type="footnotes" data-markdown="${escapeHtml(source)}"><ol>${items}</ol></section>`;
    return `${next}\n\n${stash ? stash(section) : section}`;
}
function renderFootnoteContent(markdown) {
    ...
    for (const line of lines) {
        ...
        if (line.trim())
            html += `<p>${renderInlineFootnoteMarkdown(line.trim())}</p>`;
    }
    if (inList)
        html += "</ul>";
    return html;
}
function renderInlineFootnoteMarkdown(value) {
    return value
